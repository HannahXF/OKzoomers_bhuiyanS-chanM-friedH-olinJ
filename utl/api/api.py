...
def _info(player_id):
    return json.loads(str(urlopen(f'https://www.balldontlie.io/api/v1/players/{player_id}').read()))

# Gets the player stats from the BallDontLie API
# Returns the string of the JSON dictionary
def _stats(player_id):
    return json.loads(str(urlopen(f'https://www.balldontlie.io/api/v1/season_averages?season=2017&player_ids[]={player_id}').read()))

# Gets the player image from the NBA Player API
# Returns the image link
def _image(player_id, info):
    data = json.loads(bytes(info, 'utf-8'))
